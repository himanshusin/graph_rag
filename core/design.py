...
COMMUNITY_COLORS = [ACCENT, SUCCESS, WARN, FAINT, DANGER]
...
def stylesheet(nav_counts: Optional[Dict[str, int]] = None) -> str:
    """Full app stylesheet. Nav row icons and counts are baked in as generated CSS."""
    counts = nav_counts or {}
    nav_rules = []
    for position, (key, _label) in enumerate(NAV, start=1):
        row = (
            f'.st-key-nav [data-testid="stRadioGroup"] > div:nth-child({position}) '
            '[data-testid="stMarkdownContainer"] p'
        )
        mask = _icon_mask(NAV_ICONS[key])
        nav_rules.append(
            f"{row}::before{{-webkit-mask:{mask} center/contain no-repeat;mask:{mask} center/contain no-repeat}}"
        )
        if key in counts:
            nav_rules.append(
                f'{row}::after{{content:"{counts[key]}";margin-left:auto;'
                f"font:500 10.5px var(--k-mono);color:var(--k-faint)}}"
            )
...
:root {{
  --k-canvas:{CANVAS}; --k-panel:{PANEL}; --k-subtle:{SUBTLE}; --k-hover:{HOVER};
  --k-border:{BORDER}; --k-border-strong:{BORDER_STRONG}; --k-border-input:{BORDER_INPUT};
  --k-border-faint:{BORDER_FAINT};
  --k-text:{TEXT}; --k-muted:{MUTED}; --k-faint:{FAINT};
  --k-accent:{ACCENT}; --k-accent-soft:{ACCENT_SOFT}; --k-accent-border:{ACCENT_BORDER};
  --k-accent-bg:{ACCENT_BG};
  --k-success:{SUCCESS}; --k-success-bg:{SUCCESS_BG}; --k-warn:{WARN}; --k-warn-bg:{WARN_BG};
  --k-danger:{DANGER};
...
/* each screen fills the viewport; only the thread / content area scrolls.
   The composer is taken out of flow so it cannot push the screen into a scroll. */
[data-testid="stBottom"] {{
  position:fixed !important; left:{SIDEBAR_WIDTH}px;
  right:calc((100vw - {SIDEBAR_WIDTH}px) * {RAIL_RATIO});
  bottom:0; width:auto !important; background:transparent;
}}
.st-key-thread, [class*="st-key-pad_"] {{ height:calc(100vh - 48px); overflow-y:auto; }}

/* ---------------------------------------------------------------- sidebar */
[data-testid="stSidebar"] {{
  width:{SIDEBAR_WIDTH}px !important; min-width:{SIDEBAR_WIDTH}px !important;
  background:var(--k-panel); border-right:1px solid var(--k-border);
}}
...
.k-brand__mark {{
  width:22px; height:22px; flex:none; border-radius:6px; background:var(--k-text); color:#fff;
  font:600 11px var(--k-mono); display:grid; place-items:center;
}}
.k-brand__name {{ font-weight:600; font-size:14px; }}
.k-brand__ver {{ margin-left:auto; font:500 10px var(--k-mono); color:var(--k-faint); }}

.k-side-label {{
  padding:22px 8px 8px; margin:0; line-height:1.3;
  font:500 10.5px var(--k-mono); letter-spacing:.06em; color:var(--k-faint); text-transform:uppercase;
}}

/* sidebar radio groups rendered as nav / mode lists */
[data-testid="stSidebar"] [data-testid="stRadioGroup"] {{ gap:2px; }}
[data-testid="stSidebar"] [data-testid="stRadioOption"] {{
  padding:7px 8px; border-radius:6px; margin:0; width:100%;
}}
[data-testid="stSidebar"] [data-testid="stRadioOption"] > div {{ align-items:center; gap:0; width:100%; }}
...
[data-testid="stSidebar"] [data-testid="stRadioOption"] [data-testid="stMarkdownContainer"] p {{
  ...
  font-size:13px; line-height:1.3; color:var(--k-muted); white-space:nowrap;
}}
[data-testid="stSidebar"] [data-testid="stRadioOption"]:hover {{ background:var(--k-subtle); }}
[data-testid="stSidebar"] [data-testid="stRadioOption"][data-selected="true"] {{ background:var(--k-hover); }}
[data-testid="stSidebar"] [data-testid="stRadioOption"][data-selected="true"]
  [data-testid="stMarkdownContainer"] p {{ color:var(--k-text); font-weight:500; }}
.st-key-nav [data-testid="stRadioOption"] [data-testid="stMarkdownContainer"] p::before {{
  content:""; width:14px; height:14px; flex:none; background-color:currentColor;
}}
.st-key-modes [data-testid="stWidgetLabel"] p {{
  padding:22px 8px 8px; margin:0;
  font:500 10.5px var(--k-mono); letter-spacing:.06em; color:var(--k-faint); text-transform:uppercase;
}}
.st-key-modes [data-testid="stRadioOption"] {{ padding:6px 8px; }}
.st-key-modes [data-testid="stRadioOption"] [data-testid="stMarkdownContainer"] p {{ font-size:12px; }}
.st-key-modes [data-testid="stRadioOption"] [data-testid="stMarkdownContainer"] p::before {{
  content:""; width:6px; height:6px; flex:none; border-radius:50%;
  border:1px solid var(--k-border-input); box-sizing:border-box;
}}
.st-key-modes [data-testid="stRadioOption"][data-selected="true"]
  [data-testid="stMarkdownContainer"] p::before {{ background:var(--k-accent); border-color:var(--k-accent); }}
{"".join(nav_rules)}

/* sidebar sliders */
.st-key-controls {{ margin-top:18px; padding:0 8px; }}
.st-key-controls [data-testid="stWidgetLabel"] p {{ font-size:12px; color:var(--k-muted); }}
[data-testid="stSliderThumbValue"] p {{ font:500 11px var(--k-mono); color:var(--k-text); }}
[data-testid="stSliderTickBarMin"], [data-testid="stSliderTickBarMax"] {{ display:none; }}

.k-side-foot {{
  margin-top:22px; padding:8px; display:flex; align-items:center; gap:7px;
  font-size:11.5px; color:var(--k-success);
}}
.k-side-foot--warn {{ color:var(--k-warn); }}
...
.k-cite {{ font:500 10px var(--k-mono); color:var(--k-accent); margin-left:2px; }}
...
.k-ev {{ border:1px solid var(--k-border); border-radius:8px; padding:10px 12px; }}
.k-ev--focus {{ border-color:var(--k-accent-border); background:var(--k-accent-bg); }}
...
/* horizontal radio rendered as an underline tab bar */
[class*="st-key-tabs_"], [class*="st-key-pills_"] {{ width:auto; flex:none; }}
[class*="st-key-tabs_"] [data-testid="stRadioGroup"],
[class*="st-key-pills_"] [data-testid="stRadioGroup"] {{ flex-direction:row; flex-wrap:nowrap; }}
[class*="st-key-tabs_"] [data-testid="stRadioGroup"] > div,
[class*="st-key-pills_"] [data-testid="stRadioGroup"] > div {{ flex:none; }}
[class*="st-key-tabs_"] [data-testid="stRadioGroup"] {{ gap:18px; align-items:center; }}
[class*="st-key-tabs_"] code {{
  font:11px var(--k-mono); color:var(--k-faint); background:none; padding:0 0 0 4px;
}}
[class*="st-key-tabs_"] [data-testid="stRadioOption"] {{ padding:0; margin:0; }}
[class*="st-key-tabs_"] [data-testid="stRadioOption"] > div > div:first-child {{ display:none; }}
[class*="st-key-tabs_"] [data-testid="stRadioOption"] [data-testid="stMarkdownContainer"] p {{
  margin:0; padding:0 0 4px; font-size:12.5px; color:var(--k-muted); white-space:nowrap;
}}
[class*="st-key-tabs_"] [data-testid="stRadioOption"][data-selected="true"]
  [data-testid="stMarkdownContainer"] p {{
  color:var(--k-text); font-weight:500; box-shadow:inset 0 -2px 0 var(--k-text);
}}

/* horizontal radio rendered as a pill row */
[class*="st-key-pills_"] [data-testid="stRadioGroup"] {{ gap:6px; }}
[class*="st-key-pills_"] [data-testid="stRadioOption"] {{
  padding:4px 10px; margin:0; border-radius:999px; border:1px solid var(--k-border-strong);
}}
[class*="st-key-pills_"] [data-testid="stRadioOption"] > div > div:first-child {{ display:none; }}
[class*="st-key-pills_"] [data-testid="stRadioOption"] [data-testid="stMarkdownContainer"] p {{
  margin:0; font-size:11.5px; color:var(--k-muted); line-height:1.35; white-space:nowrap;
}}
[class*="st-key-pills_"] [data-testid="stRadioOption"][data-selected="true"] {{
  background:var(--k-text); border-color:var(--k-text);
}}
[class*="st-key-pills_"] [data-testid="stRadioOption"][data-selected="true"]
  [data-testid="stMarkdownContainer"] p {{ color:#fff; font-weight:500; }}
</style>
"""
...
def evidence_card(citation: Dict, focus: bool = False) -> str:
    ...
    return (
        f'<div class="k-ev{" k-ev--focus" if focus else ""}">'
        f'<div class="k-ev__head"><span class="k-ev__idx">{esc(citation.get("index", ""))}</span>'
