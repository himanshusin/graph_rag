...
class DocumentIngestor:
    ...
    def extract_from_pdf_bytes(pdf_bytes: bytes) -> str:
        """Extract text from PDF file bytes with high performance via PyMuPDF."""
...
class EntityRelationshipExtractor:
    ...
    def extract_chunk(self, chunk_text: str, chunk_id: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """Invoke LLM on chunk and parse entities & relationships."""
...
class GraphRAGEngine:
    ...
    def build_from_text(
        ...
        """
        Build complete GraphRAG pipeline from raw text with progress streaming.
        """
        ...
        # ---------------------------------------------------------------------
        # Step 3: Entity & Relationship Deduplication
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # Step 4: Community Detection & Graph Topology
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # Step 5: Hierarchical Community Reports
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # Step 6: Vector Ingestion into ChromaDB
        # ---------------------------------------------------------------------
        ...
        # ---------------------------------------------------------------------
        # Step 7: Export PyVis Interactive Graph & Static Image
        # ---------------------------------------------------------------------
        notify(0.97, "Generating interactive PyVis physics graph visualization...")
        self._generate_visualizations(df_nodes, df_relationships)

        elapsed = round(time.perf_counter() - start_time, 2)
        notify(1.0, f"✅ GraphRAG Engine built successfully in {elapsed}s!", {
    ...
    def _generate_visualizations(self, nodes_df: pd.DataFrame, relationships_df: pd.DataFrame):
        """Generate both static dark-themed PNG and interactive HTML graph."""
        colors = ['#FF6B6B', '#4D96FF', '#6BCB77', '#FFD93D', '#9D4EDD', '#FF9F45', '#00C9A7', '#F38181', '#E83A59', '#4E9F3D']
        
        # 1. Interactive PyVis Graph
        net = Network(height='600px', width='100%', bgcolor='#0B0C10', font_color='white', notebook=True, cdn_resources='in_line')
        ...
        if len(nodes_df) > 0:
            fig, ax = plt.subplots(figsize=(14, 9), facecolor='#0B0C10')
            ...
            nx.draw_networkx_edges(G, pos, edge_color='#2A324B', alpha=0.5, width=1.5, ax=ax)
            nx.draw_networkx_labels(G, pos, font_size=8, font_color='white', font_weight='bold', ax=ax)

            plt.title('GraphRAG Knowledge Graph (Leiden Communities)', color='white', fontsize=14, pad=15)
            plt.axis('off')
            plt.tight_layout()
